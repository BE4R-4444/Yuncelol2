...
# 初始化Flask-Limiter
limiter = Limiter(
    get_remote_address,  # key_func作为第一个位置参数
    app=app,  # app作为命名参数
...
def ranking():
    ...
    for row in c.fetchall():
        ...
        if team['logo_url'] and not team['logo_url'].startswith(('http://', 'https://')):
            team['logo_url'] = f"http://{team['logo_url']}"  # 或者使用您的基础URL
        teams.append(team)
...
@limiter.limit("5 per day")  # 每日最多5次
def start_quiz():
...
def show_question():
    ...
    if 'questions' not in session or 'current_index' not in session:
        return redirect(url_for('home'))  # 数据丢失时返回首页
...
def submit_answer():
    try:
        ...
        return jsonify({
            ...
            'correct_answer': chr(correct_answer + ord('A')),  # 转为字母
